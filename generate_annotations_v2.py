# -*- coding: utf-8 -*-
"""Generate complete annotations for all 10000 SMS rows - v2 improved"""
...
def classify_row(text, orig_label):
    ...
    collection_pats = [r'合同逾期.*全款偿还', r'分期资格.*取消',
                       ...
                       r'仲裁.*诉讼', r'提交.*仲裁', r'申请.*仲裁']
    for pat in collection_pats:
        if re.search(pat, text):
            return 'HARASS', '高', '匹配催收模式'
    
    # TRANSACTION (service notifications, valid transactions)
    ...
    # Operator service notifications (NOT marketing)
    operator_trans_kws = [
        '流量.*使用', '语音.*使用', '余额', '话费', '套餐', '月租',
        '充值', '消费', '积分.*领取', '权益.*领取', '月卡权益',
        '日包', '月包', '流量包', '语音包', '叠加包',
        '流量.*到账', '语音.*到账', '电子券.*到账',
        '流量.*使用完', '语音.*使用完',
        '余额不足', '扣费', '代交', '代扣', '充值成功',
    ...
    # Check for government/public service (NEEDS_REVIEW)
    ...
    # Operator services (流量/余额/账单 etc.)
    if operator_trans_count >= 3:
        return 'TRANSACTION', '高', '运营商服务通知（流量/余额/账单）'
