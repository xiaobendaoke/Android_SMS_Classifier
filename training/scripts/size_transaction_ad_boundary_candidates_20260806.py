#!/usr/bin/env python3
"""Count text-free candidate pools for the next TRANSACTION/AD boundary arbitration."""
...
def main() -> int:
        ...
        rows.append(
            {
                ...
                "coverage_subtype": coverage_subtype(r["text"]),
            }
        )
    ...
    def summarize(name: str, pool: list[dict]) -> dict:
        ...
        by_subtype = Counter(str(r["coverage_subtype"]) for r in pool)
        return {
            "pool": name,
            "count": len(pool),
            ...
            "coverage_subtype_counts": dict(
                sorted(by_subtype.items(), key=lambda x: (-x[1], x[0]))
            ),
        }

    report = {
        "run_id": "size_transaction_ad_boundary_candidates_20260806_r1",
