...
payload = {
    ...
    "decision_reason": (
        "The run observed NaN at epoch 18, so it is rejected even though the restored checkpoint satisfied all five gates."
        if not failed
        else "The run observed NaN at epoch 18 and the restored best checkpoint still did not satisfy all five acceptance gates on the same independently evaluated Keras zh validation run."
    ),
    ...
    "nan_observed": True,
    "nan_observed_epoch": 18,
    "nan_recovery": "Restored best validation checkpoint from epoch 13 before independent Keras evaluation.",
