#!/usr/bin/env python3
"""Run lr=5e-4 with a train-only rule-based hard-boundary weight hook."""
...
REPO = ROOT.parent
RUN = "stage2_xfyun_carrier_repayment_lr_5e4_hard_boundary_2p0_20260806_r1"
...
def main() -> int:
    ...
    cfg["training"]["hard_boundary_multiplier"] = 2.0
    ...
    result = {
        ...
        "hypothesis": (
            "Weighting only train rows that match a transaction-domain coverage "
            "subtype (OTP/LOGISTICS/ORDER/REPAYMENT/CARRIER/BANK) and carry a "
            "non-TRANSACTION label sharpens the hard AD/HARASS/FRAUD boundary "
            "without re-weighting TRANSACTION rows, targeting the CARRIER/UNMATCHED "
            "and BANK/UNMATCHED confusion structure without using validation results."
        ),
        "only_changed_variable": (
            "training.hard_boundary_multiplier=2.0 "
            "(baseline=1.0/off; mask=train-only coverage_subtype on non-TRANSACTION rows)"
        ),
