#!/usr/bin/env python3
"""Run a validation-only transaction-weight variant on the lr=5e-4 front."""
...
def main() -> int:
    ...
    cfg["training"]["class_weight_multipliers"]["TRANSACTION"] = 1.5
    ...
    result = {
        ...
        "hypothesis": (
            "A slightly higher TRANSACTION multiplier on the lr=5e-4 front may "
            "recover the remaining transaction recall gap without destroying precision."
        ),
        "only_changed_variable": (
            "training.class_weight_multipliers.TRANSACTION=1.5 (baseline=1.4; "
