#!/usr/bin/env python3
"""Run one protection-loss variant on the updated provisional overlay."""
...
def main() -> int:
    ...
    # Keep the baseline hard-label setup unchanged; vary only this loss weight.
    cfg["training"]["class_weight_multipliers"]["TRANSACTION"] = 1.4
    cfg["transaction_protection"]["loss_weight"] = 0.70
    cfg["output"]["checkpoint_dir"] = str(ARTIFACT.relative_to(ROOT))
    cfg["output"]["keras_path"] = str(
        (ARTIFACT / "sms_bytecnn_fp32.keras").relative_to(ROOT)
    )
    ...
    result = {
        ...
        "seed": 42,
        "hypothesis": (
            "Increasing only the shared transaction-protection loss weight can "
            "reduce transaction misses without inference-time category overrides."
        ),
        "only_changed_variable": "transaction_protection.loss_weight=0.70 (baseline=0.35)",
