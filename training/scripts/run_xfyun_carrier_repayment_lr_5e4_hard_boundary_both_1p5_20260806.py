#!/usr/bin/env python3
"""Run lr=5e-4 with a both-sides train-only hard-boundary weight hook."""
...
REPO = ROOT.parent
RUN = "stage2_xfyun_carrier_repayment_lr_5e4_hard_boundary_both_1p5_20260806_r1"
...
def main() -> int:
    ...
    result = {
        ...
        "hypothesis": (
            "Rounds 1-2 showed hard-negative weighting trades TRANSACTION precision "
            "for recall and harms HARASS; a both-sides mask that also weights covered "
            "TRANSACTION rows should push TRANSACTION recall toward the gate while "
            "keeping AD/FRAUD precision gains, and HARASS rows are left unweighted so "
            "its F1 stays near the lr5e4 baseline."
        ),
        "only_changed_variable": (
            "training.hard_boundary_labels=[TRANSACTION, AD, FRAUD] at multiplier 1.5 "
            "(round1 mask was non-TRANSACTION only at 2.0)"
        ),
