#!/usr/bin/env python3
"""Run a dropout=0.10 baseline experiment with stronger teacher distillation."""
...
REPO = ROOT.parent
RUN = "stage2_xfyun_carrier_repayment_dropout_0p10_distill_alpha_0p40_20260805_r1"
...
def main() -> int:
    ...
    cfg["distillation"]["alpha"] = 0.40
    cfg["distillation"]["beta"] = 0.60
    ...
    result = {
        ...
        "hypothesis": (
            "Increasing only teacher-distribution weight from the dropout=0.10 baseline may recover more "
            "TRANSACTION recall and macro structure, counterbalancing the high-precision/low-recall behavior "
            "observed when hard-label alpha was raised to 0.70."
        ),
        "only_changed_variable": "distillation.alpha=0.40, distillation.beta=0.60 (baseline=0.55/0.45)",
