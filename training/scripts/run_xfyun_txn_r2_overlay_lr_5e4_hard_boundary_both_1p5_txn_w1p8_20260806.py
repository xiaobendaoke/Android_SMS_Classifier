#!/usr/bin/env python3
"""Run TXN weight 1.8 on the second-round TRANSACTION/AD boundary arbitration overlay."""
...
REPO = ROOT.parent
RUN = "stage2_xfyun_txn_r2_overlay_lr_5e4_hard_boundary_both_1p5_txn_w1p8_20260806_r1"
ARTIFACT = ROOT / "artifacts" / "experiments" / RUN
DATA = ROOT / "data" / "processed_txn_boundary_r2_arbitration_20260806_r1"
...
def main() -> int:
    ...
    cfg["data"]["train_manifest"] = (
        "data/processed_txn_boundary_r2_arbitration_20260806_r1/train.jsonl"
    )
    cfg["data"]["val_manifest"] = (
        "data/processed_txn_boundary_r2_arbitration_20260806_r1/validation.jsonl"
    )
    ...
    result = {
        ...
        "hypothesis": (
            "Round 6 (TXN weight 1.8) is the best candidate so far. The remaining "
            "TXN recall gap matches 18 validation TRANSACTION->AD misses. This "
            "round arbitrates all 160 no-exclusion TRANSACTION/AD model-boundary "
            "rows (including 19 TRANSACTION misses and 141 AD rows the model calls "
            "TXN/FRAUD/HARASS) so label corrections can fix both recall and "
            "precision without changing training settings."
        ),
        "only_changed_variable": (
            "data manifest=processed_txn_boundary_r2_arbitration_20260806_r1 "
            "(second-round TRANSACTION/AD boundary arbitration overlay; all round6 "
