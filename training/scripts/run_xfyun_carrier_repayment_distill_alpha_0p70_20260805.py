#!/usr/bin/env python3
"""Run a validation-only distillation loss-mix experiment."""
...
REPO = ROOT.parent
RUN = "stage2_xfyun_carrier_repayment_distill_alpha_0p70_20260805_r1"
...
def main() -> int:
    ...
    cfg["distillation"]["alpha"] = 0.70
    cfg["distillation"]["beta"] = 0.30
    ...
    result = {
        ...
        "seed": 42,
        "hypothesis": (
            "Increasing the hard-label share of the distillation loss may recover "
            "HARASS and macro-F1 boundary quality while retaining enough teacher "
            "signal for FRAUD recall."
        ),
        "only_changed_variable": (
            "distillation loss mix alpha/beta=0.70/0.30 "
            "(baseline=0.55/0.45)"
        ),
