#!/usr/bin/env python3
"""One-variable learning-rate experiment on the provisional xfyun overlay."""
...
RUN="stage2_xfyun_overlay_lr_5e4_20260804_r1"; ART=T/"artifacts/experiments"/RUN; REP=T/"reports/experiments"/RUN
...
def main():
 ...
 cfg["seed"]=42; cfg["data"]["train_manifest"]="data/processed_xfyun_ai_annotation_20260802_r1/train.jsonl"; cfg["data"]["val_manifest"]="data/processed_xfyun_error_relabel_20260803_r1/validation.jsonl"; cfg["training"]["class_weight_multipliers"]["TRANSACTION"]=1.4; cfg["training"]["learning_rate"]=5.0e-4; cfg["output"]["checkpoint_dir"]=str(ART.relative_to(T)); cfg["output"]["keras_path"]=str((ART/"sms_bytecnn_fp32.keras").relative_to(T))
 ...
 out={"run_id":RUN,"status":"EXPLORATORY_PROVISIONAL_VALIDATION_ONLY","claim_allowed":False,"formal_acceptance_allowed":False,"locked_test_read":False,"quantization_run":False,"android_export_run":False,"seed":42,"hypothesis":"Lowering only Adam learning rate prevents late-epoch instability and improves validation generalization.","only_changed_variable":"training.learning_rate=0.0005 (baseline=0.001)","config_sha256":sha(c),"returncode":rc}; m=ART/"distill_manifest.json"
