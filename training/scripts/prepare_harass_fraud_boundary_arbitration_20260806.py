#!/usr/bin/env python3
"""Prepare blind packs for stacked HARASS/FRAUD model-boundary arbitration."""
...
BASE = ROOT / "data" / "processed_harass_boundary_arbitration_20260806_r1"
PREV_LABELS = BASE / "provisional_labels.jsonl"
RUN = "harass_fraud_boundary_arbitration_20260806_r1"
...
EXPECTED_SHA = {
    "train": "9aba63d1a18f767875571bced66245fd9cca10fd0aeba893afea6c8d77797795",
    "validation": "56e980f025944a357ac265f6ecc0fdafd1d5f6647eb4072023f75df6e31cd216",
}
...
def main() -> int:
    ...
    if not PREV_LABELS.exists():
        raise SystemExit(f"missing previous arbitration labels: {PREV_LABELS}")
    if not MODEL.exists():
    ...
    prev_ids = {r["id"] for r in load_records(PREV_LABELS)}
    ...
    selected = [
        ...
        and pred != r["label"]
        and r["id"] not in prev_ids
    ]
    if not selected:
        raise SystemExit("no new model-boundary candidates after excluding previous round")
    ...
    manifest = {
        ...
        "candidate_selection": "zh_harass_fraud_model_boundary_misclassified",
