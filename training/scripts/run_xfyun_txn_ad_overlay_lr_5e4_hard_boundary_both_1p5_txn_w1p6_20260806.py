#!/usr/bin/env python3
"""Run TXN class weight 1.6 on the stacked TRANSACTION/AD overlay with round4 settings."""
...
REPO = ROOT.parent
RUN = "stage2_xfyun_txn_ad_overlay_lr_5e4_hard_boundary_both_1p5_txn_w1p6_20260806_r1"
...
def main() -> int:
    ...
    result = {
        ...
        "seed": 42,
        "baseline": "stage2_xfyun_txn_ad_overlay_lr_5e4_hard_boundary_both_1p5_20260806_r1",
        "hypothesis": (
            "Round 3 left only TRANSACTION recall below the gate (0.9329 vs 0.985). "
            "Raising the TRANSACTION class-weight multiplier from 1.4 to 1.6 on the "
            "same stacked overlay should push recall upward; precision has headroom "
            "(0.9373) so the tradeoff can absorb some precision loss."
        ),
        "only_changed_variable": (
            "training.class_weight_multipliers.TRANSACTION 1.4 -> 1.6 "
