    ...
    run_all.write_text(
        "\n".join(
            [
                ...
                "cd \"$ROOT\"",
                "failed=0",
                f"bash \"$ROOT/{run_pass_a_rel}\"; rc_a=$?",
                f"bash \"$ROOT/{run_pass_b_rel}\"; rc_b=$?",
                "export PYTHONPATH=\"$ROOT/training\"",
                f"\"$ROOT/.venv/bin/python\" \"$ROOT/training/scripts/reconcile_ai_dual_pass_20260806.py\" --run-dir \"$ROOT/{out_rel}\" --stage merge-ab || failed=1",
                f"bash \"$ROOT/{run_pass_c_rel}\" || failed=1",
                f"\"$ROOT/.venv/bin/python\" \"$ROOT/training/scripts/reconcile_ai_dual_pass_20260806.py\" --run-dir \"$ROOT/{out_rel}\" --stage merge-c || failed=1",
