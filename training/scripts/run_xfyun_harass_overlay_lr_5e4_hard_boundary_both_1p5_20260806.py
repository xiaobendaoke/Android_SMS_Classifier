#!/usr/bin/env python3
"""Run round4 lr=5e-4 hard-only both-sides 1.5 config on the AI-arbitrated overlay data."""
...
REPO = ROOT.parent
RUN = "stage2_xfyun_harass_overlay_lr_5e4_hard_boundary_both_1p5_20260806_r1"
ARTIFACT = ROOT / "artifacts" / "experiments" / RUN
DATA = ROOT / "data" / "processed_harass_boundary_arbitration_20260806_r1"
...
def main() -> int:
    ...
    cfg["data"]["train_manifest"] = (
        "data/processed_harass_boundary_arbitration_20260806_r1/train.jsonl"
    )
    cfg["data"]["val_manifest"] = (
        "data/processed_harass_boundary_arbitration_20260806_r1/validation.jsonl"
    )
    ...
    result = {
        ...
        "seed": 42,
        "baseline": "stage2_xfyun_carrier_repayment_lr_5e4_hard_boundary_both_1p5_20260806_r1",
        "hypothesis": (
            "Round4 was the best rejected run. The only new variable is the "
            "AI-arbitrated template-consistent overlay: 22 rows in 6 template "
            "groups were re-labeled by multi-pass AI arbitration (11 FRAUD "
            "labels moved to AD/HARASS), which should improve HARASS F1 and "
            "FRAUD boundary consistency without changing training hyperparameters."
        ),
        "only_changed_variable": (
            "data manifest=processed_harass_boundary_arbitration_20260806_r1 "
            "(all round4 training settings carried forward unchanged)"
        ),
