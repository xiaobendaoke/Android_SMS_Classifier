...
run_id="stage2_xfyun_carrier_repayment_lr_3e4_20260805_r1"
template="$repo_root/training/scripts/run_xfyun_carrier_repayment_protection_loss_0p70_20260805.py"
dst="run_xfyun_carrier_repayment_lr_3e4_20260805.py"
...
sed \
  ...
  -e 's/cfg\["transaction_protection"\]\["loss_weight"\] = 0.70/cfg["training"]["learning_rate"] = 3e-4/' \
  -e 's/Increasing only the shared transaction-protection loss weight can/Reducing only the student learning rate to 3e-4 can/' \
  -e 's/reduce transaction misses without inference-time category overrides./test whether extra optimizer stability improves precision and HARASS F1 without changing data or inference-time category overrides./' \
  -e 's/transaction_protection.loss_weight=0.70 (baseline=0.35)/training.learning_rate=3e-4 (baseline=1e-3)/' \
