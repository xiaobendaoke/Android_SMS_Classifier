#!/usr/bin/env python3
"""Run lr=5e-4 with the hard-boundary mask restricted to AD/FRAUD rows."""
...
REPO = ROOT.parent
RUN = "stage2_xfyun_carrier_repayment_lr_5e4_hard_boundary_ad_fraud_2p0_20260806_r1"
...
def main() -> int:
    ...
    cfg["training"]["hard_boundary_labels"] = ["AD", "FRAUD"]
    ...
    result = {
        ...
        "hypothesis": (
            "Round1 at multiplier 2.0 lifted TRANSACTION precision past 0.92 but "
            "dragged HARASS F1 down; restricting the same train-only mask to AD/FRAUD "
            "coverage rows should keep the precision/recall gains without over-rotating "
            "HARASS BANK/UNMATCHED examples."
        ),
        "only_changed_variable": (
            "training.hard_boundary_labels=[AD, FRAUD] vs round1 all non-TRANSACTION "
            "labels (multiplier kept at 2.0)"
        ),
