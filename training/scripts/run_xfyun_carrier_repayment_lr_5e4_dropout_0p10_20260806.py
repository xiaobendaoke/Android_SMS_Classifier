#!/usr/bin/env python3
"""Run lr=5e-4 with lower dropout as a single-variable follow-up."""
...
REPO = ROOT.parent
RUN = "stage2_xfyun_carrier_repayment_lr_5e4_dropout_0p10_20260806_r1"
...
def main() -> int:
    ...
    result = {
        ...
        "hypothesis": (
            "Lowering only dropout on the lr=5e-4 hard-label front may recover "
            "TRANSACTION precision and HARASS recall while preserving the macro F1 "
            "and FRAUD recall stability observed at the lower learning rate."
        ),
        "only_changed_variable": "model.dropout=0.10 (baseline lr=5e-4 run used 0.20)",
