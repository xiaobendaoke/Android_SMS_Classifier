#!/usr/bin/env python3
"""Run carrier/repayment TXN positive weight 2.0 on top of TXN weight 1.6."""
...
REPO = ROOT.parent
RUN = "stage2_xfyun_txn_ad_overlay_lr_5e4_hard_boundary_both_1p5_txn_w1p6_carrier_pos2p0_20260806_r1"
...
def main() -> int:
    ...
    cfg["training"]["class_weight_multipliers"]["TRANSACTION"] = 1.6
    cfg["training"]["carrier_repayment_positive_multiplier"] = 2.0
    ...
    result = {
        ...
        "hypothesis": (
            "Round 4 raised TXN recall to 0.9448 with class weight 1.6. Error "
            "clusters show TRANSACTION->AD misses concentrate in CARRIER/UNMATCHED "
            "coverage. Weighting carrier/repayment TRANSACTION positives 2.0 should "
            "close the remaining recall gap while class weight 1.6 keeps precision "
            "above the gate."
        ),
        "only_changed_variable": (
            "training.carrier_repayment_positive_multiplier 1.0 -> 2.0 "
            "(TXN class weight 1.6 and data manifest carried forward unchanged)"
        ),
        "carried_forward_from_baseline": {
            "training.learning_rate": 5e-4,
            "training.class_weight_multipliers.TRANSACTION": 1.6,
