#!/usr/bin/env python3
"""One-variable exploratory ByteCNN class-weight experiment on the xfyun overlay."""
...
RUN_ID = "stage2_xfyun_overlay_txn_weight_1p4_20260803_r1"
...
def main() -> int:
    ...
    result = {"run_id": RUN_ID, "status": "EXPLORATORY_PROVISIONAL_VALIDATION_ONLY", "claim_allowed": False, "formal_acceptance_allowed": False, "locked_test_read": False, "quantization_run": False, "android_export_run": False, "seed": 42, "hypothesis": "Increasing only the TRANSACTION class multiplier from 1.1 to 1.4 improves recall without unacceptable regressions.", "only_changed_variable": "training.class_weight_multipliers.TRANSACTION=1.4", "config_sha256": sha256(config), "data_sha256": {name: sha256(DATA / f"{name}.jsonl") for name in ("train", "validation")}, "returncode": completed.returncode}
