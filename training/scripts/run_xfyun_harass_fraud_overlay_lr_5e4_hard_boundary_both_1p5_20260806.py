#!/usr/bin/env python3
"""Run round4 config on the stacked HARASS/FRAUD model-boundary arbitration overlay."""
...
REPO = ROOT.parent
RUN = "stage2_xfyun_harass_fraud_overlay_lr_5e4_hard_boundary_both_1p5_20260806_r1"
ARTIFACT = ROOT / "artifacts" / "experiments" / RUN
DATA = ROOT / "data" / "processed_harass_fraud_boundary_arbitration_20260806_r1"
...
def main() -> int:
    ...
    cfg["data"]["train_manifest"] = (
        "data/processed_harass_fraud_boundary_arbitration_20260806_r1/train.jsonl"
    )
    cfg["data"]["val_manifest"] = (
        "data/processed_harass_fraud_boundary_arbitration_20260806_r1/validation.jsonl"
    )
    ...
    result = {
        ...
        "seed": 42,
        "baseline": "stage2_xfyun_harass_overlay_lr_5e4_hard_boundary_both_1p5_20260806_r1",
        "hypothesis": (
            "Round 2 of the data-arbitration direction: stack 100 model-boundary "
            "misclassified zh HARASS/FRAUD rows (arbitrated A/B/C, 53 corrections, "
            "2 quarantine) on top of the 22-row template-inconsistency overlay while "
            "keeping all round4 training settings unchanged. This should further "
            "improve HARASS F1 and FRAUD boundary consistency."
        ),
        "only_changed_variable": (
            "data manifest=processed_harass_fraud_boundary_arbitration_20260806_r1 "
            "(stacked model-boundary arbitration overlay; all training settings unchanged)"
        ),
