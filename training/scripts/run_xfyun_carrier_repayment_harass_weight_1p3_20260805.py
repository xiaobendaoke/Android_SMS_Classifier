#!/usr/bin/env python3
"""Run a validation-only HARASS class-weight distillation experiment."""
...
REPO = ROOT.parent
RUN = "stage2_xfyun_carrier_repayment_harass_weight_1p3_20260805_r1"
...
def main() -> int:
    ...
    cfg["training"]["class_weight_multipliers"]["HARASS"] = 1.3
    ...
    result = {
        ...
        "seed": 42,
        "hypothesis": (
            "Increasing only the HARASS hard-label multiplier to 1.3 may improve "
            "HARASS separation and macro F1 without materially lowering the "
            "TRANSACTION precision gate."
        ),
        "only_changed_variable": (
            "training.class_weight_multipliers.HARASS=1.3 (baseline=1.0; "
            "distillation teacher and TRANSACTION multiplier remain fixed)"
        ),
