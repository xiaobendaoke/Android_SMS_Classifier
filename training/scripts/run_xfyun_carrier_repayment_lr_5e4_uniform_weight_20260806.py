#!/usr/bin/env python3
"""Run lr=5e-4 with uniform supervised class weights."""
...
REPO = ROOT.parent
RUN = "stage2_xfyun_carrier_repayment_lr_5e4_uniform_weight_20260806_r1"
...
def main() -> int:
    ...
    cfg["training"]["class_weight_strategy"] = "uniform"
    ...
    result = {
        ...
        "hypothesis": (
            "Using uniform supervised class weights on the lr=5e-4 hard-label front "
            "may remove the balanced-weight downweighting of TRANSACTION and AD, "
            "testing whether transaction recall can recover without losing HARASS/FRAUD gates."
        ),
        "only_changed_variable": (
            "training.class_weight_strategy=uniform "
            "(baseline=balanced; learning_rate kept at 5e-4)"
        ),
    ...
    if manifest_path.exists():
        ...
        result["training_manifest_class_weights"] = manifest.get("class_weights")
