#!/usr/bin/env python3
"""Run conv_filters 128 on the best stacked overlay with round6 settings."""
...
REPO = ROOT.parent
RUN = "stage2_xfyun_txn_ad_overlay_lr_5e4_hard_boundary_both_1p5_txn_w1p8_conv128_20260806_r1"
...
def main() -> int:
    ...
    result = {
        ...
        "hypothesis": (
            "Round 6 (TXN weight 1.8, hard-only) is the best candidate. Adding "
            "convolutional capacity (conv_filters 80 -> 128) should let the ByteCNN "
            "separate more TRANSACTION/AD boundary patterns and close the remaining "
            "recall gap without changing any training setting."
        ),
        "only_changed_variable": (
            "model.conv_filters 80 -> 128 "
            "(data and all round6 training settings unchanged)"
        ),
