#!/usr/bin/env python3
"""Text-free validation error clustering across selected Chinese candidates."""
...
RUN_ID = "stage2_xfyun_carrier_repayment_error_clusters_20260806_r1"
...
def analyze_one(name: str, model_path: Path, records: Sequence[SmsRecord]) -> Tuple[Dict[str, object], List[str]]:
...
def consensus_analysis(
    ...
    always_wrong: List[Tuple[SmsRecord, str]] = []
    any_wrong: List[Tuple[SmsRecord, str]] = []
    signature_counts: Counter = Counter()
    for index, record in enumerate(records):
        preds = [model_predictions[name][index] for name in model_names]
        wrong_flags = [prediction != record.label for prediction in preds]
        signature = "|".join(f"{name}:{pred}" for name, pred in zip(model_names, preds))
        if any(wrong_flags):
            any_wrong.append((record, signature))
            signature_counts[f"{record.label}=>{signature}"] += 1
        if all(wrong_flags):
            always_wrong.append((record, signature))
    always_by_label = defaultdict(list)
    for record, signature in always_wrong:
        always_by_label[record.label].append((record, signature))
    return {
        "model_names": model_names,
        "any_model_wrong_count": len(any_wrong),
        "all_models_wrong_count": len(always_wrong),
        "all_models_wrong_by_true_label": {
            label: cluster_rows(rows)
            for label, rows in sorted(always_by_label.items())
        },
        "top_prediction_signatures": top_counts(signature_counts, 20),
    }
...
def main() -> int:
    ...
    parser.add_argument("--model", required=True, action="append", type=parse_model_arg)
    args = parser.parse_args()

    report = args.report_root / RUN_ID
    if report.exists():
        raise SystemExit(f"refusing to overwrite run_id: {RUN_ID}")
    for _, model_path in args.model:
    ...
    for name, model_path in args.model:
        payload, predictions = analyze_one(name, model_path, records)
    ...
    result = {
        ...
        "consensus": consensus_analysis(records, model_predictions),
        "decision": "analysis_only_no_candidate",
        "decision_reason": (
            "This report clusters validation errors without raw text or IDs. It does not "
            "create a candidate, change labels, read locked test, or authorize human review."
        ),
    }
    ...
    print(
        json.dumps(
            {
                "run_id": RUN_ID,
                "models": [payload["name"] for payload in model_payloads],
                "all_models_wrong_count": result["consensus"]["all_models_wrong_count"],
            },
            ensure_ascii=False,
        )
    )
