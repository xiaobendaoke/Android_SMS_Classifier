#!/usr/bin/env python3
"""Run a dropout=0.10 baseline experiment with lower hard-negative boundary weight."""
...
REPO = ROOT.parent
RUN = "stage2_xfyun_carrier_repayment_dropout_0p10_hard_neg_1p5_20260805_r1"
...
def main() -> int:
    ...
    cfg["distillation"]["transaction_hard_negative_multiplier"] = 1.5
    ...
    result = {
        ...
        "hypothesis": (
            "Lowering only the hard-negative transaction boundary multiplier from the dropout=0.10 baseline may "
            "reduce over-penalization of transaction-like examples and recover TRANSACTION recall while measuring "
            "whether precision, HARASS F1, macro F1, and FRAUD recall remain near gate."
        ),
        "only_changed_variable": "distillation.transaction_hard_negative_multiplier=1.5 (baseline=3.0)",
