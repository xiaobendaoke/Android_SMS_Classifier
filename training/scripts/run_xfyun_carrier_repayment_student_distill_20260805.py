#!/usr/bin/env python3
"""Distill a ByteCNN student from the newly trained local Chinese teacher."""
...
REPO = ROOT.parent
RUN = "stage2_xfyun_carrier_repayment_student_distill_20260805_r1"
...
def main() -> int:
    ...
    completed = subprocess.run([sys.executable, str(ROOT / "scripts" / "distill_student.py"), "--config", str(config), "--seed", "42"], cwd=REPO, env={**os.environ, "PYTHONPATH": str(ROOT)}, check=False)
    ...
    result = {
        ...
        "hypothesis": "Replacing hard-only training with logits from the locally trained Chinese BERT teacher improves the student's category separation.",
        "only_changed_variable": "distillation.teacher_manifest=stage2_xfyun_carrier_repayment_teacher_20260805_r1",
