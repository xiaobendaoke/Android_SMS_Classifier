#!/usr/bin/env python3
"""One-variable effective transaction class-weight experiment without clipping."""
from __future__ import annotations
import copy, hashlib, json, os, subprocess, sys
from pathlib import Path
import yaml
T=Path(__file__).resolve().parent.parent; R=T.parent
RUN="stage2_xfyun_overlay_txn_weight_1p4_no_clip_20260804_r1"; ART=T/"artifacts/experiments"/RUN; REP=T/"reports/experiments"/RUN
...
def main():
 ...
 cfg["seed"]=42; cfg["data"]["train_manifest"]="data/processed_xfyun_ai_annotation_20260802_r1/train.jsonl"; cfg["data"]["val_manifest"]="data/processed_xfyun_error_relabel_20260803_r1/validation.jsonl"; cfg["training"]["class_weight_multipliers"]["TRANSACTION"]=1.4; cfg["training"]["class_weight_clip"]=None; cfg["output"]["checkpoint_dir"]=str(ART.relative_to(T)); cfg["output"]["keras_path"]=str((ART/"sms_bytecnn_fp32.keras").relative_to(T))
 ...
 out={"run_id":RUN,"status":"EXPLORATORY_PROVISIONAL_VALIDATION_ONLY","claim_allowed":False,"formal_acceptance_allowed":False,"locked_test_read":False,"quantization_run":False,"android_export_run":False,"seed":42,"hypothesis":"Removing only class-weight clipping lets the approved transaction multiplier influence the transaction-versus-advertising boundary.","only_changed_variable":"training.class_weight_clip=null","config_sha256":sha(c),"returncode":rc}; m=ART/"distill_manifest.json"
