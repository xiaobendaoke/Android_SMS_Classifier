#!/usr/bin/env python3
"""Audit single-variable logit-bias postprocessing on fixed zh validation."""
...
RUN_ID = "stage2_xfyun_carrier_repayment_logit_bias_audit_20260805_r1"
...
def best_candidates(logits: np.ndarray, y_true: Sequence[str], label: str, values: Iterable[float]) -> Dict[str, object]:
    ...
    for value in values:
        ...
        row["min_gate_margin"] = min_gate_margin(metric)
        rows.append(row)
        if passes(metric):
            feasible.append(row)

    best_min_margin = max(rows, key=lambda row: (row["min_gate_margin"], row["macro_f1"]))
    best_precision_guarded = max(
        rows,
        key=lambda row: (
    ...
    return {
        ...
        "best_min_gate_margin": best_min_margin,
        "best_with_transaction_precision_guard": best_precision_guarded,
        "best_with_transaction_recall_guard": best_recall_guarded,
    }
...
def audit_model(model_path: Path, records, bias_values: List[float]) -> Dict[str, object]:
    ...
    encoded = np.asarray(
        [encode_text(normalize_text(text), length=max_bytes) for text in texts],
        dtype=np.int32,
    )
    ...
    return {
        ...
        "single_variable_sweeps": [
            best_candidates(logits, y_true, "TRANSACTION", bias_values),
            best_candidates(logits, y_true, "HARASS", bias_values),
            best_candidates(logits, y_true, "FRAUD", bias_values),
        ],
    }
...
def main() -> int:
    ...
    result = {
        ...
        "hypothesis": "A single fixed class-logit bias might recover one gate tradeoff, but must pass all five zh validation gates on the same candidate to be useful.",
        "search_space": {
            "bias_labels": ["TRANSACTION", "HARASS", "FRAUD"],
            "bias_min": -2.0,
            "bias_max": 2.0,
            "bias_step": 0.05,
    ...
    result["feasible_models"] = [
        {
            "model_path": model["model_path"],
            "sweep": sweep,
        }
        for model in result["models"]
        for sweep in model["single_variable_sweeps"]
        if sweep["feasible_count"] > 0
    ]
    result["decision"] = "rejected" if not result["feasible_models"] else "candidate_found"
    result["decision_reason"] = (
        "No audited single-variable logit-bias sweep produced one candidate that satisfied all five zh validation gates."
        if not result["feasible_models"]
        else "At least one single-variable logit-bias point satisfied all five zh validation gates and requires independent fixed-candidate evaluation."
    )
    (report / "audit.json").write_text(json.dumps(result, ensure_ascii=False, indent=2) + "\n", encoding="utf-8")
    print(json.dumps({"run_id": RUN_ID, "decision": result["decision"], "feasible_count": len(result["feasible_models"])}, ensure_ascii=False))
    return 0
