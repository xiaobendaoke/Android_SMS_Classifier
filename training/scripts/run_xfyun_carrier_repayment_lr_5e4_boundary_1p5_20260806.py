#!/usr/bin/env python3
"""Run lr=5e-4 with carrier/repayment boundary weighting."""
...
REPO = ROOT.parent
RUN = "stage2_xfyun_carrier_repayment_lr_5e4_boundary_1p5_20260806_r1"
...
def main() -> int:
    ...
    cfg["data"]["train_manifest"] = (
        "data/processed_xfyun_carrier_repayment_relabel_20260804_r1/train.jsonl"
    )
    cfg["data"]["val_manifest"] = (
        "data/processed_xfyun_carrier_repayment_relabel_20260804_r1/validation.jsonl"
    )
    cfg["training"]["learning_rate"] = 5e-4
    cfg["training"]["class_weight_multipliers"]["TRANSACTION"] = 1.4
    cfg["training"]["carrier_repayment_boundary_multiplier"] = 1.5
    ...
    result = {
        ...
        "seed": 42,
        "baseline": "stage2_xfyun_carrier_repayment_lr_5e4_20260805_r1",
        "hypothesis": (
            "Weighting both sides of the predefined carrier/repayment boundary may "
            "improve separation between true TRANSACTION rows and lookalike AD/HARASS/FRAUD "
            "rows without biasing the boundary entirely toward TRANSACTION."
        ),
        "only_changed_variable": (
            "training.carrier_repayment_boundary_multiplier=1.5 "
            "(baseline=1.0; learning_rate kept at 5e-4)"
        ),
    ...
    if manifest_path.exists():
        ...
        result["carrier_repayment_boundary_count"] = manifest.get(
            "carrier_repayment_boundary_count"
        )
        result["carrier_repayment_positive_count"] = manifest.get(
            "carrier_repayment_positive_count"
        )
    ...
    print(
        json.dumps(
            {"run_id": RUN, "returncode": completed.returncode, "has_manifest": manifest_path.exists()},
            ensure_ascii=False,
        )
    )
