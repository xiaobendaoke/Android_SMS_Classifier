#!/usr/bin/env python3
"""Run TXN class weight 2.0 on the stacked TRANSACTION/AD overlay with round4 settings."""
...
REPO = ROOT.parent
RUN = "stage2_xfyun_txn_ad_overlay_lr_5e4_hard_boundary_both_1p5_txn_w2p0_20260806_r1"
...
def main() -> int:
    ...
    cfg["training"]["class_weight_multipliers"]["TRANSACTION"] = 2.0
    ...
    result = {
        ...
        "hypothesis": (
            "Round 6 raised TXN recall to 0.9568 with class weight 1.8 while "
            "precision held at 0.9258. Raising the multiplier to 2.0 should push "
            "recall toward the gate; if precision drops below 0.92 the next round "
            "switches to remaining TXN boundary arbitration instead."
        ),
        "only_changed_variable": (
            "training.class_weight_multipliers.TRANSACTION 1.8 -> 2.0 "
