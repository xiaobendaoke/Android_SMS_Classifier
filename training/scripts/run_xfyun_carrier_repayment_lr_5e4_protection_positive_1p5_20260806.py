#!/usr/bin/env python3
"""Run lr=5e-4 with stronger transaction-protection positive weighting."""
...
def main() -> int:
    ...
    cfg["transaction_protection"]["positive_weight"] = 1.5
    ...
    result = {
        ...
        "hypothesis": (
            "Increasing only transaction_protection.positive_weight may strengthen "
            "the shared representation for true TRANSACTION rows, testing whether "
            "transaction misses fall without changing four-class thresholds or "
            "inference-time category routing."
        ),
        "only_changed_variable": (
            "transaction_protection.positive_weight=1.5 "
            "(baseline=1.10; learning_rate kept at 5e-4)"
        ),
