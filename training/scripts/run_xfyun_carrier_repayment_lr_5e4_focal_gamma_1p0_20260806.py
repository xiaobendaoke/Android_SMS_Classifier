#!/usr/bin/env python3
"""Run primary focal gamma 1.0 on the lr=5e-4 front."""
...
REPO = ROOT.parent
RUN = "stage2_xfyun_carrier_repayment_lr_5e4_focal_gamma_1p0_20260806_r1"
...
TEACHER = (
    ...
    / "artifacts"
    / "experiments"
    / "stage2_xfyun_carrier_repayment_teacher_20260805_r1"
...
def main() -> int:
    ...
    cfg["distillation"]["primary_focal_gamma"] = 1.0
    ...
    result = {
        ...
        "hypothesis": (
            "Increasing primary focal gamma from 0.50 to 1.0 on the lr=5e-4 front may "
            "focus updates on hard boundary examples strongly enough to recover FRAUD recall "
            "and HARASS F1 without the NaN instability seen in the dropout follow-up."
        ),
        "only_changed_variable": (
            "distillation.primary_focal_gamma=1.0 (baseline=0.50; "
            "learning_rate kept at 5e-4 from the current front)"
        ),
