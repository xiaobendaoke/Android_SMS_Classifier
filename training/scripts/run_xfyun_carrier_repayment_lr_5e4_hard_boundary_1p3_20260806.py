#!/usr/bin/env python3
"""Run lr=5e-4 with a lighter train-only rule-based hard-boundary weight hook."""
...
REPO = ROOT.parent
RUN = "stage2_xfyun_carrier_repayment_lr_5e4_hard_boundary_1p3_20260806_r1"
...
def main() -> int:
    ...
    cfg["training"]["hard_boundary_multiplier"] = 1.3
    ...
    result = {
        ...
        "hypothesis": (
            "The 2.0 hard-boundary hook lifted TRANSACTION precision past 0.92 but "
            "dragged HARASS F1 down; lowering the same train-only coverage-subtype "
            "multiplier to 1.3 should keep part of the precision gain without "
            "over-rotating the HARASS BANK/UNMATCHED boundary."
        ),
        "only_changed_variable": (
            "training.hard_boundary_multiplier=1.3 "
            "(round1=2.0; mask definition unchanged)"
        ),
