#!/usr/bin/env python3
"""Run lr=5e-4 with the transaction-protection head/loss disabled."""
...
def main() -> int:
    ...
    cfg["model"]["transaction_protection_head"] = False
    cfg["transaction_protection"]["enabled"] = False
    ...
    result = {
        ...
        "hypothesis": (
            "Disabling only the transaction-protection auxiliary head and loss on the "
            "lr=5e-4 hard-label front may let the four-class primary head optimize "
            "TRANSACTION/HARASS/AD/FRAUD separation directly, testing whether the "
            "auxiliary protection objective is hurting acceptance-gate tradeoffs."
        ),
        "only_changed_variable": (
            "model.transaction_protection_head=false and transaction_protection.enabled=false "
            "(baseline=true; learning_rate kept at 5e-4)"
        ),
    ...
    if manifest_path.exists():
        ...
        result["transaction_protection"] = manifest.get("transaction_protection")
