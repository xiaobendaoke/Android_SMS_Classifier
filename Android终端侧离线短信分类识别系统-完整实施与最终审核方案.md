...
> 核心约束：短信不上云、端侧额外内存 ≤100 MB、单条全链路时延 ≤500 ms、事务召回率 ≥98%、**当前阶段仅要求中文**（英/印地/印尼暂缓，不作为本期验收硬指标）。  
...
13. 不要擅自增加 GAN、LLM 端侧推理、多语言模型常驻、联网更新等非必要功能；本期不要把英/印地/印尼验收重新加回硬指标。
...
| 语种范围 | 中文 | 中文独立报告；英/印地/印尼不纳入本期硬验收 |
...
| 风险 | 发现方式 | 降级/修复 |
|---|---|---|
| 中文数据不足 | 数据审计 | 先交付工程闭环，报告不得宣称准确率达标；英/印地/印尼本期不纳入硬验收 |
...
4. 用蒸馏后的 Byte TextCNN 获得中文场景下的低内存、易 INT8 部署能力（多语扩展暂缓）。
5. 用结构化剪枝和量化源码证明模型压缩过程。
6. 用分类与处置分离避免“低置信全判事务”的指标取巧。
7. 用冻结数据、真机报告和最终审核脚本证明所有指标。

只有当最终审核中不存在 P0/P1 问题，且在 4GB/6GB 真机上同时满足内存、时延、事务召回（中文）、对抗和离线要求，项目才可以标记为正式完成。
