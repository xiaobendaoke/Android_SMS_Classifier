#!/usr/bin/env python3
"""Annotate all 600 records in annotator_A CSV."""
import csv
import sys

csv.field_size_limit(sys.maxsize)

INPATH = r"training/data/interim/annotation/transaction_specialist/transaction_specialist_annotator_A.csv"
...
with open(INPATH, "r", encoding="utf-8-sig") as f:
    ...
    raw_fieldnames = [fn.strip('"') for fn in reader.fieldnames]
...
# All IDs from the "联通助理" (China Unicom Assistant) notifications - these are TRANSACTION (missed call/blocking notifications)
cu_assistant_ids = [
...
for rid in cu_assistant_ids:
    set_label(rid, "TRANSACTION")

# Telecom plan/usage notifications - TRANSACTION
telco_usage_ids = [
    ...
    "zh-n2w-06121", "zh-n2w-06224", "zh-n2w-06746", "zh-n2w-06779",
    "zh-n2w-06942", "zh-n2w-07083", "zh-n2w-07352", "zh-n2w-08251",
    "zh-n2w-08270", "zh-n2w-08756", "zh-n2w-09028", "zh-n2w-09129",
    "zh-n2w-09203", "zh-n2w-09239", "zh-n2w-09644", "zh-n2w-09765",
    "zh-n2w-02364", "zh-n2w-08207", "zh-n2w-08597", "zh-n2w-08962",
    "zh-n2w-09260", "zh-n2w-09344", "zh-n2w-09997", "zh-n2w-09998",
]
# Most are TRANSACTION; a few with heavy AD content get AD
for rid in telco_usage_ids:
    if rid in ("zh-n2w-07352", "zh-n2w-08756", "zh-n2w-06121", "zh-n2w-09203"):
        set_label(rid, "AD")
    else:
        set_label(rid, "TRANSACTION")

# Re-assign specific telco AD ones
set_label("zh-n2w-07352", "AD", "话费流量查询附带大量增值业务推广")
set_label("zh-n2w-08756", "AD", "卡券到期提醒+活动推广")
set_label("zh-n2w-06121", "AD", "服务提醒+签到福利推广")
set_label("zh-n2w-09203", "AD", "余量查询服务推广")

# Gitcode telecom/usage notifications - TRANSACTION
gitcode_telco_ids = [
...
# Most are TRANSACTION (usage/status); a few with heavy AD content
for rid in gitcode_telco_ids:
    if rid in ("zh_02415", "zh_00614", "zh_04899"):
        set_label(rid, "TRANSACTION")  # Purchase notification
    elif rid in ("zh_00788",):
        set_label(rid, "TRANSACTION")  # Lottery result notification
    elif rid in ("zh_01094", "zh_07284"):
        set_label(rid, "TRANSACTION")  # Login notification
    elif rid in ("zh_04231",):
        set_label(rid, "TRANSACTION")  # Missed call notification
    elif rid in ("zh_04695",):
        set_label(rid, "TRANSACTION")  # Broadband maintenance notice
    else:
        set_label(rid, "TRANSACTION")

# Product/order/ecommerce notifications - TRANSACTION
order_n2w_ids = [
    "zh-n2w-00235", "zh-n2w-00354", "zh-n2w-00397", "zh-n2w-00439",
    "zh-n2w-00446", "zh-n2w-01021", "zh-n2w-01665", "zh-n2w-01783",
    "zh-n2w-02336", "zh-n2w-02713", "zh-n2w-02787", "zh-n2w-02958",
    "zh-n2w-03326", "zh-n2w-03393", "zh-n2w-03929", "zh-n2w-04024",
    "zh-n2w-04869", "zh-n2w-05241", "zh-n2w-05467", "zh-n2w-05555",
    "zh-n2w-06048", "zh-n2w-06389", "zh-n2w-07381", "zh-n2w-07504",
    "zh-n2w-07694", "zh-n2w-07902", "zh-n2w-07940", "zh-n2w-08584",
]
for rid in order_n2w_ids:
    if rid in ("zh-n2w-00439", "zh-n2w-05241", "zh-n2w-07504"):
        set_label(rid, "AD")  # Heavy promotion/service marketing
    else:
        set_label(rid, "TRANSACTION")

# Payment/repayment notifications - TRANSACTION
payment_ids = [
...
# Most are TRANSACTION; some with harassment language
for rid in payment_ids:
    if rid in ("zh-n2w-00553", "zh-n2w-00986", "zh-n2w-02711",
               "zh-n2w-08605", "zh-n2w-05228", "zh-n2w-07118",
               "zh-n2w-07282", "zh-n2w-06947", "zh-n2w-08058",
               "zh-n2w-08741"):
        set_label(rid, "TRANSACTION")  # Still transaction (repayment reminders)
    else:
        set_label(rid, "TRANSACTION")

# Gitcode repayment/bill notifications - TRANSACTION
gitcode_payment_ids = [
...
for rid in gitcode_payment_ids:
    if rid in ("zh_07400", "zh_05282"):
        set_label(rid, "AD")  # Heavy promotion for installment
    else:
        set_label(rid, "TRANSACTION")

# ===== GROUP 1: Bank transaction notifications (TRANSACTION) =====
bank_tx_ids = [
...
for rid in bank_tx_ids:
    set_label(rid, "TRANSACTION")

# ===== GROUP 2: Gitcode order/travel/reservation notifications (TRANSACTION) =====
order_ids = [
...
# These 7 IDs were found missing from the automated check\nset_label(\"zh_01157\", \"TRANSACTION\", \"浦发外币记账规则通知\")\nset_label(\"zh_05356\", \"TRANSACTION\", \"北京银行借记卡开通快捷支付\")\nset_label(\"zh_05773\", \"TRANSACTION\", \"政府一站通启动码\")\nset_label(\"zh-n2w-03977\", \"FRAUD\", \"游戏激活诈骗链接\")\nset_label(\"zh-n2w-04389\", \"TRANSACTION\", \"聚水潭系统授权验证码通知\")\nset_label(\"zh-n2w-01141\", \"FRAUD\", \"冒充银行通知兑换手机钓鱼链接\")\nset_label(\"zh-n2w-02538\", \"FRAUD\", \"可疑还款链接冒充金融平台\")\n\n# ===== GROUP 3: OTP / Verification codes (TRANSACTION) =====
otp_ids = [
    "zh-n2w-00013", "zh-n2w-00470", "zh-n2w-02645",
    "zh-n2w-02829", "zh-n2w-05036", "zh-n2w-06698",
    "zh-n2w-07241", "zh-n2w-09364",
...
set_label("zh_09418", "NEEDS_REVIEW", "短信截断，内容不完整")
set_label("zh_06773", "NEEDS_REVIEW", "验证码格式异常")
set_label("zh_10339", "TRANSACTION", "支付宝校验码（乱码）")

# ===== GROUP 4: Delivery / Logistics notifications (TRANSACTION) =====
delivery_ids = [
...
for rid in delivery_ids:
    set_label(rid, "TRANSACTION")

# ===== GROUP 5: AD (Commercial promotions) =====
ad_ids = [
    ...
    "zh-n2w-03612", "zh-n2w-04159", "zh-n2w-05310",
    "zh-n2w-05978", "zh-n2w-06392", "zh-n2w-07611",
    # order/shopping ads
    "zh_03551", "zh_08836", "zh_08993",
    "zh_05491", "zh_06477",
]
for rid in ad_ids:
    set_label(rid, "AD")

# ===== GROUP 6: HARASS (Harassment / grey loans) =====
harass_ids = [
    "zh-n2w-00848", "zh-n2w-04080", "zh-n2w-05697", "zh-n2w-08429",
    "zh-n2w-09995",
    "zh_01409", "zh_04429", "zh_05580",
    "zh_05155",
    "zh_01214",
]
for rid in harass_ids:
    set_label(rid, "HARASS")

# ===== GROUP 7: NEEDS_REVIEW =====
review_ids = [
    "zh-n2w-04550", "zh_00713", "zh_05480",
    "zh_08484", "zh_09139",
    "zh-n2w-00627", "zh-n2w-01426", "zh-n2w-01922",
    "zh-n2w-02003", "zh-n2w-03254", "zh-n2w-03612",
    "zh-n2w-07202", "zh-n2w-08277",
    "zh_09470",
]
for rid in review_ids:
    set_label(rid, "NEEDS_REVIEW")

# Notes for NEEDS_REVIEW and other special cases
notes_map = {
    "zh-n2w-04550": "满意度调查，非四分类意图",
    "zh_00713": "续短信内容不完整",
    "zh_05480": "服务通知征求同意，非四分类",
    "zh_08484": "仅查询余额，语义模糊",
    "zh_09139": "错误提示，非四分类",
    "zh-n2w-00627": "满意度评价邀请，非四分类",
    "zh-n2w-01426": "通话体验评价邀请，非四分类",
    "zh-n2w-01922": "满意度评价邀请，非四分类",
    "zh-n2w-02003": "宽带业务评价邀请，非四分类",
    "zh-n2w-03254": "学校互动课堂通知，非四分类",
    "zh-n2w-03612": "业务营销评价邀请，非四分类",
    "zh-n2w-07202": "满意度调查，非四分类",
    "zh-n2w-08277": "业务订购体验评价，非四分类",
    "zh_09470": "满意度评价邀请，非四分类",
    "zh_09418": "短信截断，内容不完整",
    "zh_06773": "验证码格式异常",
    "zh-n2w-01141": "冒充银行兑换诈骗链接",
    "zh-n2w-02538": "可疑还款链接，冒充平台",
    "zh-n2w-03977": "游戏诈骗激活链接",
}
...
print(f"Default NEEDS_REVIEW: {len(rows) - count_labeled}")
